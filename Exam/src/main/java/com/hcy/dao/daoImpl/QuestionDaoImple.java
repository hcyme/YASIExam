...
import com.hcy.dao.QuestionDao;
	...
	public int addQuestionToDB(Question question) {
		...
		if (question != null) {
			...
			try {
				...
				StringBuffer sql = new StringBuffer();
				sql.append(
						"insert into Question(q_id,Q_Type,Q_Title,Q_Select,Q_Score,Q_Answer,course_id,Q_Explaination) values (?,?,?,?,?,?,?,?)");
		...
		if (questions != null && questions.size() > 0) {
			...
			for (int i = 0; i < questions.size(); i++) {
				...
				q_id = questions.get(i).getQ_id() != null
						? ("".equals(questions.get(i).getQ_id()) ? null : questions.get(i).getQ_id())
						: null;
