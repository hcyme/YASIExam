			...
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i> <a href="jsp/teacher/index.jsp">首页</a><span
						class="crumb-step">&gt;</span><span class="crumb-name">批改试卷</span>
				</div>
			</div>

			<div class="result-wrap">
				<form id="form"
					action="servlet/ScoreCorrectServlet.do?operation=correct"
					method="post">
					...
					<c:forEach var="subAnswer" items="${subAnswerList}"
						...
						<fieldset>
							...
							考试答案<br />
							<textarea rows="5" cols="50" name="answer">${subAnswer.answer}</textarea>
							<br /> 参考答案<br />
							<textarea rows="5" cols="50" name="rightAnswer"
								readonly="readonly">${subAnswer.rightAnswer}</textarea>
							<br /> 评分：<input type="text" name="${subAnswer.q_id}" size="10"
								id="score" />(总分值:<span id="q_score">${subAnswer.q_score }</span>)
						</fieldset>
						<div class="bk20"></div>
					</c:forEach>
					<div class="inputArea" style="text-align: center">
						<input type="submit" id="submit" style="margin-top: 20px;"
							class="button_blue" value="提交评分" />
					</div>
				</form>
			</div>
		</div>
