	...
	$(
			function() {
				...
				function inint() {
					...
					$
							.post(
									"PaperManageServlet?operation=findSelectedQuestion",
									{
										"qids" : qidArray2
									},
									function(data, status) {
										if (status == "success") {
											...
											for (var i = 0; i < questionList.length; i++) {
												console
														.log(questionList[i].q_type);
												var tr = $("<tr>"
														+ "<td>"
														+ (i + 1)
														+ "</td>"
														+ "<td>"
														+ (questionList[i].q_type == 'q1' ? '单选题'
																: questionList[i].q_type == 'q2' ? '多选题'
																		: questionList[i].q_type == 'q3' ? '填空题'
																				: questionList[i].q_type == 'q4' ? '判断题'
																						: questionList[i].q_type == 'q5' ? '主观题'
																								: '')
														+ "</td>"
														+ "<td>"
														+ questionList[i].q_title
														+ "</td>"
														+ "<td name='scoreTd'>"
														+ questionList[i].q_score
														+ "</td>"
				...
				$("#ctype")
						.ready(
								function() {
									$
											.get(
													"PaperManageServlet?operation=courseType",
													function(date) {
														var couresList = JSON
																.parse(date);
														for (var i = 0; i < couresList.length; i++) {
															var option = $("<option value="+couresList[i].c_id+">"
																	+ couresList[i].c_name
																	+ "</option>");
															$("#ctype").append(
																	option);
				...
				$("#qtype")
						.ready(
								function() {
									$
											.get(
													"PaperManageServlet?operation=questionType",
													function(date) {
														var questionList = JSON
																.parse(date);
														for (var i = 0; i < questionList.length; i++) {
															var option = $("<option value="+questionList[i].id+">"
																	+ questionList[i].qt_name
																	+ "</option>");
															$("#qtype").append(
																	option);
														}
				...
				var findQuestion = function() {
					$("#qTable1").empty();
					var head = $("<tr>" + "<th width='15%'>试题序号</th>"
							+ "<th width='15%'>题目类型</th>"
							+ "<th width='50%'>题目</th>"
							+ "<th width='10%'>分值</th>"
					...
					$
							.get(
									"PaperManageServlet?operation=findQuestion&courseType="
											+ $("#ctype").val()
											+ "&questionType="
											+ $("#qtype").val(),
									function(date) {
										var questionList = JSON.parse(date);
										var q_id2 = $("#qTable2").find(
												"input[name='q_id']");
										for (var i = 0; i < questionList.length; i++) {
											var flag = 1;
											q_id2
				...
				$(".insert-tab").on(
						"click",
						"table input[name='add']",
						function() {
							var qTr = $(this).parent().parent();
							qTr.children("input[name='q_id']").attr("name",
									"q_ids");
							$("#qTable2").append(qTr);
				...
				$(".insert-tab").on(
						"click",
						"table input[name='del']",
						function() {
							var qTr = $(this).parent().parent();
							qTr.children("input[name='q_ids']").attr("name",
									"q_id");
							$("#qTable1").append(qTr);
				...
				<ul class="sidebar-list">
					<li>
						<ul class="sub-menu">
							...
							<li><a href="jsp/teacher/news_list.jsp"><i
							...
							<li class="on"><a href="PaperManageServlet?operation=update"><i
							...
							<li><a href="jsp/teacher/test_score_list.jsp"><i
									class="icon-font">&#xe005;</i>成绩统计</a></li>
						</ul>
					</li>
				</ul>
			</div>
			...
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i> <a href="jsp/teacher/index.jsp">首页</a><span
						...
						href="jsp/teacher/test_list.jsp">考试管理</a><span class="crumb-step">&gt;</span><span>修改考试</span>
				</div>
			</div>
			<div class="result-wrap">
				<div class="result-content">
					<form
						action="PaperManageServlet?operation=modify&pid=${paper.p_id}"
						method="post" id="myform" name="myform">
						<table class="insert-tab" width="100%">
							<tbody>
								<tr>
									<th>考试班级</th>
									<td><select name="classes" class="required">
											<option value="0">请选择班级</option>
											<c:forEach items="${classesList}" var="classes">
												<option value="${classes.cls_id }"
													${classes.cls_id==paper.cls_id?'selected':''}>${classes.cls_name}</option>
											</c:forEach>
									</select></td>
								</tr>
								<tr>
									<th width="120"><i class="require-red">*</i>所属课程：</th>
									<td><select name="course" id="ctype">
											<c:forEach items="${coursList }" var="course">
												<option value="${course.c_id }"
													${course.c_id==paper.c_id?'selected':''}>${course.c_name }</option>
											</c:forEach>
									</select></td>
								</tr>
								<tr>
									...
									<td><input class="common-text required" id="title"
										name="papername" size="50" value="${paper.p_name}" type="text">
									</td>
								</tr>
								<tr>
									<th style="text-align: center" colspan="2">试题列表</th>
								</tr>
								<tr>
									<th width="120">题目类型：</th>
									<td><select name="q_type" id="qtype" class="required">
								...
								<tr>
									<th style="text-align: center" colspan="2">已选试题</th>
								<tr>
								<tr>
									<td colspan="2">
										<div style="overflow-y: scroll;">
								...
								<tr>
									...
									<td><input class="common-text required" id="title"
										name="score" size="50" value="${paper.p_scores}" type="text" />
									</td>
								</tr>
								<tr>
									<th><i class="require-red">*</i>考试日期：</th>
									<td><input class="common-text required" id="title"
										name="p_date" size="50" value="${paper.p_date }" type="date" />
									</td>
								</tr>
								<tr>
									<th><i class="require-red">*</i>考试开始时间：</th>
									<td><input class="common-text required" id="title"
										name="startTime" size="50" value="${ paper.startTime}"
										type="time" /></td>
								</tr>
								<tr>
									<th><i class="require-red">*</i>考试结束时间：</th>
									<td><input class="common-text required" id="title"
										name="endTime" size="50" value="${ paper.endTime}" type="time">
									</td>
								</tr>
								<tr>
									<th></th>
